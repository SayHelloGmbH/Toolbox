...
export default class ImageSelectorWithPlaceholder extends Component {
	...
	constructor( props ) {
		...
		this.url_attribute_key = externalURLAttribute || 'externalURL';
		this.labels_object = labels || {};

		this.externalURL_attribute = attributes[ this.url_attribute_key ];
	...
	render() {
		...
		const externalURL = this.props.attributes[this.url_attribute_key];

		return (

			<Fragment>
				{ (!!image.id || !!externalURL) && !this.state.is_uploading &&
					<BlockControls>
						<Toolbar>
							<MediaReplaceFlow
								name={_x('Bild ersetzen', 'MediaReplaceFlow label', 'sha')}
								mediaId={image.id}
								mediaURL={this.externalURL_attribute}
