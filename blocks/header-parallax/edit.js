...
import { RichText, InspectorControls, withColors, ContrastChecker, getColorClassName, PanelColorSettings } from '@wordpress/block-editor';
...
class Edit extends Component {
	...
	render() {
		...
		const { attributes, className, colors, setAttributes, setGradientColor, setTextColor } = this.props;
			...
			<Fragment>
				...
				<section className={wrapperClassName}>
					<div className={`${classNameBase}__inner`}>
						{ !!image.id &&
							<LazyImage className={`${className}__figure`} image={image} background={false} admin={true}/>
						}
						{ !image.id &&
							<div className={`${className}__figure ${className}__figure--empty`}></div>
						}
						<div className={`${classNameBase}__content`}>
							<RichText
								tagName='h1'
								placeholder={_x('Geben Sie einen Titel ein', 'Field placeholder', 'sha')}
							...
							<RichText
								tagName='div'
								placeholder={_x('Geben Sie einen Text ein', 'Field placeholder', 'sha')}
