...
registerBlockType( 'sht/header-parallax', {
	...
	attributes: {
		...
		buttonText: {
			type: 'string',
			default: _x( 'Button-Text', 'Default button text', 'sha' ),
		},
		buttonUrl: {
			type: 'string',
			default: '#',
	...
	save( { attributes } ) {
		...
		const { align, text, title, image, gradientColor, textColor, buttonText, buttonUrl } = attributes;
		...
		return (
			<section className={wrapperClassName} data-parallax="scroll" data-position="center top" data-bleed="0" data-image-src={ parallax.src } data-natural-width={ parallax.width } data-natural-height={ parallax.height }>
				<div className={`${classNameBase}__inner`}>
					{ !!image.id &&
						<LazyImage className={`${classNameBase}__figure`} image={image} background={false} admin={false}/>
					}
					{ !image.id &&
						<div className={`${classNameBase}__figure ${classNameBase}__figure--empty`}></div>
					}
					<div className={`${classNameBase}__content`}>
						{ !!title && <RichText.Content tagName="h1" className={`${classNameBase}__title`} value={ title } /> }
						{ !!text && <RichText.Content tagName="div" className={`${classNameBase}__text`} value={ text } /> }
					</div>
				</div>
			</section>
		);
